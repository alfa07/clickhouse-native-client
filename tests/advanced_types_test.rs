...
//!
//! These tests verify support for advanced ClickHouse types including Nothing, Decimal128,
//! IPv4/IPv6, Geo types, and Int128/UInt128.
//!
//! ## Prerequisites
//! 1. Start ClickHouse server: `just start-db`
//! 2. Run tests: `cargo test --test advanced_types_test -- --ignored --nocapture`
//!
//! ## Test Coverage
...
async fn test_nothing_type() {
    ...
    let result = client.query("SELECT NULL AS nothing_col").await.expect("Query failed");
...
async fn test_decimal128() {
    ...
    client.query("INSERT INTO test_decimal128 VALUES (12345678901234.56789)")
        .await
        .expect("INSERT failed");

    // Query back
    let result = client.query("SELECT * FROM test_decimal128").await.expect("SELECT failed");
...
async fn test_ipv4_column() {
    ...
    client.query("CREATE TABLE IF NOT EXISTS test_ipv4 (ip IPv4) ENGINE = Memory")
        .await
        .expect("Failed to create table");
    ...
    // Query back
    let result = client.query("SELECT * FROM test_ipv4 ORDER BY ip").await.expect("SELECT failed");
...
async fn test_ipv6_column() {
    ...
    client.query("CREATE TABLE IF NOT EXISTS test_ipv6 (ip IPv6) ENGINE = Memory")
        .await
        .expect("Failed to create table");
    ...
    // Query back
    let result = client.query("SELECT * FROM test_ipv6").await.expect("SELECT failed");
...
async fn test_point_type() {
    ...
    let result = client.query("SELECT (1.5, 2.5) AS point").await.expect("Query failed");
...
async fn test_polygon_type() {
    ...
    let query = "SELECT [[(0.0, 0.0), (1.0, 0.0), (0.5, 1.0), (0.0, 0.0)]] AS polygon";
...
async fn test_int128_column() {
    ...
    // Query back
    let result = client.query("SELECT * FROM test_int128 ORDER BY val").await.expect("SELECT failed");
...
async fn test_uint128_column() {
    ...
    client.query("INSERT INTO test_uint128 VALUES (0), (1), (18446744073709551615)")
        .await
        .expect("INSERT failed");

    // Query back
    let result = client.query("SELECT * FROM test_uint128 ORDER BY val").await.expect("SELECT failed");
...
async fn test_nullable_nothing() {
    ...
    // Test Nullable(Nothing) - ClickHouse doesn't allow this in table definitions
    // This test verifies that we correctly handle the expected error
    ...
    assert!(result.is_err(), "Expected CREATE TABLE with Nullable(Nothing) to fail");
    ...
    if let Err(error) = result {
        ...
        assert!(error_msg.contains("Nullable(Nothing)") || error_msg.contains("cannot be used in tables"),
            "Error should mention Nullable(Nothing) restriction, got: {}", error_msg);
    }

    // However, SELECT NULL (which returns Nullable(Nothing)) should work fine
    let result = client.query("SELECT NULL AS nothing_col").await.expect("SELECT NULL should work");
...
async fn test_mixed_advanced_types() {
    ...
    // Query back
    let result = client.query("SELECT * FROM test_mixed_types ORDER BY id").await.expect("SELECT failed");

    println!("Mixed advanced types query returned {} rows", result.total_rows());
