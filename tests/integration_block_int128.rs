/// Integration tests for Int128 column using Block insertion
mod common;

use clickhouse_native_client::{
    column::numeric::ColumnInt128,
    Block,
};
use common::{
...
#[tokio::test]
#[ignore]
async fn test_int128_block_insert_basic() {
    let (mut client, db_name) =
        create_isolated_test_client("int128_block_basic")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (value Int128) ENGINE = Memory",
            db_name
        ))
        .await
...
#[tokio::test]
#[ignore]
async fn test_int128_block_insert_boundary() {
    let (mut client, db_name) =
        create_isolated_test_client("int128_block_boundary")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (id UInt32, value Int128) ENGINE = Memory",
            db_name
        ))
        .await
        .expect("Failed to create table");
...
proptest! {
    ...
    #[test]
    #[ignore]
    fn test_int128_block_insert_random(values in prop::collection::vec(any::<i128>(), 1..100)) {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let (mut client, db_name) = create_isolated_test_client("int128_block_random")
                .await
                .expect("Failed to create test client");
