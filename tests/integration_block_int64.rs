/// Integration tests for Int64 column using Block insertion
mod common;

use clickhouse_client::{
    column::numeric::ColumnInt64,
    types::Type,
    Block,
};
use common::{
...
#[tokio::test]
#[ignore]
async fn test_int64_block_insert_basic() {
    let (mut client, db_name) =
        create_isolated_test_client("int64_block_basic")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (value Int64) ENGINE = Memory",
            db_name
        ))
        .await
    ...
    col.append(-9223372036854775808);
    col.append(9223372036854775807);
    block
    ...
    let mut expected =
        vec![1000000000, -9223372036854775808, 9223372036854775807];
    expected
...
#[tokio::test]
#[ignore]
async fn test_int64_block_insert_boundary() {
    let (mut client, db_name) =
        create_isolated_test_client("int64_block_boundary")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (id UInt32, value Int64) ENGINE = Memory",
            db_name
        ))
        .await
        .expect("Failed to create table");
    ...
    let mut val_col = ColumnInt64::new(Type::int64());
...
proptest! {
    ...
    #[test]
    #[ignore]
    fn test_int64_block_insert_random(values in prop::collection::vec(any::<i64>(), 1..100)) {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let (mut client, db_name) = create_isolated_test_client("int64_block_random")
                .await
                .expect("Failed to create test client");
