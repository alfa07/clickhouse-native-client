...
//!
//! These tests verify query ID, parameters, settings, and tracing context features.
//!
//! ## Prerequisites
//! 1. Start ClickHouse server: `just start-db`
//! 2. Run tests: `cargo test --test query_features_test -- --ignored --nocapture`
//!
//! ## Test Coverage
...
use clickhouse_client::{Client, ClientOptions, Query, TracingContext};
...
async fn test_query_id_tracking() {
    ...
    let query_id = format!("test_qid_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
    ...
    let result = client.query(check_query).await.expect("Failed to query query_log");

    println!("Query ID {} found in query_log: {} rows", query_id, result.total_rows());
}
...
async fn test_query_parameters() {
    ...
    let result = client.query(query).await.expect("Parameterized query failed");
...
async fn test_tracing_context() {
    ...
    println!("Query with tracing context returned {} rows", result.total_rows());
    assert_eq!(result.total_rows(), 1);
}
...
async fn test_query_id_with_insert() {
    ...
    let query_id = format!("test_insert_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());

    // INSERT with query ID
    let insert_query = Query::new("INSERT INTO test_qid_insert VALUES (1, 'Alice'), (2, 'Bob')")
        .with_query_id(&query_id);
    ...
    // Verify
    let result = client.query("SELECT count(*) FROM test_qid_insert").await.expect("SELECT failed");
    assert_eq!(result.total_rows(), 1);
...
async fn test_settings_max_threads() {
    ...
    let query = Query::new("SELECT sleep(0.001) FROM system.numbers LIMIT 100")
...
async fn test_parameter_null_value() {
    ...
    // This test may fail depending on ClickHouse version support for NULL parameters
    match result {
        Ok(r) => println!("NULL parameter query returned {} rows", r.total_rows()),
...
async fn test_multiple_parameters() {
    ...
    let result = client.query(query).await.expect("Multi-parameter query failed");
...
async fn test_combined_features() {
    ...
    let query_id = format!("test_combined_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
    ...
    let result = client.query(query).await.expect("Combined features query failed");

    println!("Combined query {} returned {} rows", query_id, result.total_rows());
    assert_eq!(result.total_rows(), 1);
}
...
async fn test_settings_readonly() {
    ...
    let result = client.query(query).await.expect("Query with readonly failed");
    assert_eq!(result.total_rows(), 1);

    // Try to create table with readonly=1 (should fail if enforced)
    let create_query = Query::new("CREATE TABLE IF NOT EXISTS test_readonly (x UInt64) ENGINE = Memory")
        .with_setting("readonly", "1");
