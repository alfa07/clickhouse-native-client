/// Integration tests for Int32 column using Block insertion
mod common;

use clickhouse_client::{
    column::numeric::ColumnInt32,
    types::Type,
    Block,
};
use common::{
...
#[tokio::test]
#[ignore]
async fn test_int32_block_insert_basic() {
    let (mut client, db_name) =
        create_isolated_test_client("int32_block_basic")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (value Int32) ENGINE = Memory",
            db_name
        ))
        .await
    ...
    let mut expected = vec![100000, -2147483648, 2147483647];
    expected
...
#[tokio::test]
#[ignore]
async fn test_int32_block_insert_boundary() {
    let (mut client, db_name) =
        create_isolated_test_client("int32_block_boundary")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (id UInt32, value Int32) ENGINE = Memory",
            db_name
        ))
        .await
        .expect("Failed to create table");
    ...
    let mut val_col = ColumnInt32::new(Type::int32());
...
proptest! {
    ...
    #[test]
    #[ignore]
    fn test_int32_block_insert_random(values in prop::collection::vec(any::<i32>(), 1..100)) {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let (mut client, db_name) = create_isolated_test_client("int32_block_random")
                .await
                .expect("Failed to create test client");
