/// Integration tests for UInt32 column using Block insertion
mod common;

use clickhouse_client::{
    column::numeric::ColumnUInt32,
    types::Type,
    Block,
};
use common::{
...
#[tokio::test]
#[ignore]
async fn test_uint32_block_insert_basic() {
    let (mut client, db_name) =
        create_isolated_test_client("uint32_block_basic")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (value UInt32) ENGINE = Memory",
            db_name
        ))
        .await
    ...
    let mut expected = vec![100000, 0, 4294967295];
    expected
...
#[tokio::test]
#[ignore]
async fn test_uint32_block_insert_boundary() {
    let (mut client, db_name) =
        create_isolated_test_client("uint32_block_boundary")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (id UInt32, value UInt32) ENGINE = Memory",
            db_name
        ))
        .await
        .expect("Failed to create table");
    ...
    for (idx, (_desc, value)) in test_cases.iter().enumerate() {
        ...
        let mut val_col = ColumnUInt32::new(Type::uint32());
...
proptest! {
    ...
    #[test]
    #[ignore]
    fn test_uint32_block_insert_random(values in prop::collection::vec(any::<u32>(), 1..100)) {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let (mut client, db_name) = create_isolated_test_client("uint32_block_random")
                .await
                .expect("Failed to create test client");
