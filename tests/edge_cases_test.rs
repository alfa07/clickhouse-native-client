...
async fn test_column_names_with_spaces() {
    ...
    client
        .insert("test_special_names", block)
        .await
        .expect("Failed to insert");

    // Query back with quoted names
    let result = client
        .query(r#"SELECT `column with spaces`, `another name` FROM test_special_names"#)
        .await
...
async fn test_column_names_with_unicode() {
    ...
    client
        .insert("test_unicode_names", block)
        .await
        .expect("Failed to insert");

    // Query back
    let result = client
        .query(r#"SELECT `数字`, `текст`, `🚀emoji` FROM test_unicode_names"#)
        .await
        .expect("Failed to query");
...
async fn test_column_names_sql_keywords() {
    ...
    // Insert data
    client
        .query(r#"INSERT INTO test_keyword_names (`select`, `from`, `where`) VALUES (1, 'a', 'b')"#)
        .await
        .expect("Failed to insert");

    // Query back
    let result = client
        .query(r#"SELECT `select`, `from`, `where` FROM test_keyword_names"#)
        .await
        .expect("Failed to query");
...
async fn test_null_parameter_in_query() {
    ...
    // Query NULL values
    let result = client
        .query("SELECT id, value FROM test_null_param WHERE value IS NULL")
        .await
        .expect("Failed to query");
    ...
    let result = client
        .query("SELECT id, value FROM test_null_param WHERE value IS NOT NULL")
        .await
        .expect("Failed to query");
...
async fn test_type_mismatch_exception() {
    ...
    let result = client
        .query("INSERT INTO test_type_mismatch_edge VALUES ('not_a_number')")
        .await;

    // This should either fail or be caught by ClickHouse type conversion
    // The exact error depends on ClickHouse version, so we just check it
    // doesn't panic
    match result {
...
async fn test_empty_string_values() {
    ...
    client
        .insert("test_empty_strings", block)
        .await
        .expect("Failed to insert");

    // Query back
    let result = client
        .query("SELECT id, text FROM test_empty_strings ORDER BY id")
        .await
        .expect("Failed to query");
...
async fn test_very_long_column_name() {
    ...
    // Create column name at or near the limit (ClickHouse supports up to 256
    // characters)
