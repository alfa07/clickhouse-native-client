/// Integration tests for UInt64 column using Block insertion
mod common;
...
#[tokio::test]
#[ignore]
async fn test_uint64_block_insert_basic() {
    let (mut client, db_name) =
        create_isolated_test_client("uint64_block_basic")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (value UInt64) ENGINE = Memory",
            db_name
        ))
        .await
    ...
    col.append(0);
    col.append(18446744073709551615);
    block
    ...
    let mut expected = vec![1000000000, 0, 18446744073709551615];
    expected
...
#[tokio::test]
#[ignore]
async fn test_uint64_block_insert_boundary() {
    let (mut client, db_name) =
        create_isolated_test_client("uint64_block_boundary")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (id UInt32, value UInt64) ENGINE = Memory",
            db_name
        ))
        .await
        .expect("Failed to create table");
...
proptest! {
    ...
    #[test]
    #[ignore]
    fn test_uint64_block_insert_random(values in prop::collection::vec(any::<u64>(), 1..100)) {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let (mut client, db_name) = create_isolated_test_client("uint64_block_random")
                .await
                .expect("Failed to create test client");
