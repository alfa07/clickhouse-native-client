/// Integration tests for Int16 column using Block insertion
mod common;
...
#[tokio::test]
#[ignore]
async fn test_int16_block_insert_basic() {
    let (mut client, db_name) =
        create_isolated_test_client("int16_block_basic")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (value Int16) ENGINE = Memory",
            db_name
        ))
        .await
    ...
    let mut expected = vec![1000, -32768, 32767];
    expected
...
#[tokio::test]
#[ignore]
async fn test_int16_block_insert_boundary() {
    let (mut client, db_name) =
        create_isolated_test_client("int16_block_boundary")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (id UInt32, value Int16) ENGINE = Memory",
            db_name
        ))
        .await
        .expect("Failed to create table");
    ...
    let mut val_col = ColumnInt16::new(Type::int16());
...
proptest! {
    ...
    #[test]
    #[ignore]
    fn test_int16_block_insert_random(values in prop::collection::vec(any::<i16>(), 1..100)) {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let (mut client, db_name) = create_isolated_test_client("int16_block_random")
                .await
                .expect("Failed to create test client");
