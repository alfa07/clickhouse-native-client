/// Integration tests for UInt128 column using Block insertion
mod common;

use clickhouse_client::{
    column::numeric::ColumnUInt128,
    types::Type,
    Block,
};
use common::{
...
#[tokio::test]
#[ignore]
async fn test_uint128_block_insert_basic() {
    let (mut client, db_name) =
        create_isolated_test_client("uint128_block_basic")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (value UInt128) ENGINE = Memory",
            db_name
        ))
        .await
    ...
    col.append(0);
    col.append(340282366920938463463374607431768211455);
    block
    ...
    let mut expected =
        vec![1000000000000, 0, 340282366920938463463374607431768211455];
    expected
...
#[tokio::test]
#[ignore]
async fn test_uint128_block_insert_boundary() {
    let (mut client, db_name) =
        create_isolated_test_client("uint128_block_boundary")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (id UInt32, value UInt128) ENGINE = Memory",
            db_name
        ))
        .await
        .expect("Failed to create table");
    ...
    let test_cases = vec![
        ...
        ("Test value", 1000000000000),
    ];

    let mut block = Block::new();
    let mut id_col =
        clickhouse_client::column::numeric::ColumnUInt32::new(Type::uint32());
    let mut val_col = ColumnUInt128::new(Type::uint128());
...
proptest! {
    ...
    #[test]
    #[ignore]
    fn test_uint128_block_insert_random(values in prop::collection::vec(any::<u128>(), 1..100)) {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let (mut client, db_name) = create_isolated_test_client("uint128_block_random")
                .await
                .expect("Failed to create test client");
