...
#[tokio::test]
#[ignore]
async fn test_nullable_array_ipv6() {
    let (mut client, db_name) =
        create_isolated_test_client("nullable_array_ipv6")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (ips Nullable(Array(IPv6))) ENGINE = Memory",
            db_name
        ))
        .await
        .expect("Failed to create table");

    // Use SQL for complex nested types
    client
        .query(format!(
            "INSERT INTO {}.test_table VALUES (NULL), (['::1', '::2']), (NULL), (['fe80::1'])",
            db_name
        ))
        .await
        .expect("Failed to insert");
    ...
    let result = client
        .query(format!("SELECT ips FROM {}.test_table", db_name))
        .await
        .expect("Failed to select");
...
#[tokio::test]
#[ignore]
async fn test_nullable_tuple_ipv6_ipv4() {
    let (mut client, db_name) =
        create_isolated_test_client("nullable_tuple_ip")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (ip_pair Nullable(Tuple(IPv6, IPv4))) ENGINE = Memory",
            db_name
        ))
        .await
        .expect("Failed to create table");
    ...
    client
        .query(format!(
            "INSERT INTO {}.test_table VALUES (NULL), (('::1', '127.0.0.1')), (NULL), (('fe80::1', '192.168.1.1'))",
            db_name
        ))
        .await
        .expect("Failed to insert");
    ...
    let result = client
        .query(format!("SELECT ip_pair FROM {}.test_table", db_name))
        .await
        .expect("Failed to select");
