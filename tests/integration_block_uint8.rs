/// Integration tests for UInt8 column using Block insertion
mod common;

use clickhouse_client::{
    column::numeric::ColumnUInt8,
    types::Type,
    Block,
};
use common::{
...
#[tokio::test]
#[ignore]
async fn test_uint8_block_insert_basic() {
    let (mut client, db_name) =
        create_isolated_test_client("uint8_block_basic")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (value UInt8) ENGINE = Memory",
            db_name
        ))
        .await
...
#[tokio::test]
#[ignore]
async fn test_uint8_block_insert_boundary() {
    let (mut client, db_name) =
        create_isolated_test_client("uint8_block_boundary")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (id UInt32, value UInt8) ENGINE = Memory",
            db_name
        ))
        .await
        .expect("Failed to create table");
    ...
    let mut val_col = ColumnUInt8::new(Type::uint8());
...
proptest! {
    ...
    #[test]
    #[ignore]
    fn test_uint8_block_insert_random(values in prop::collection::vec(any::<u8>(), 1..100)) {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let (mut client, db_name) = create_isolated_test_client("uint8_block_random")
                .await
                .expect("Failed to create test client");
