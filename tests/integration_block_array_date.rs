/// Integration tests for Array(Date) column using Block insertion
mod common;
...
#[tokio::test]
#[ignore]
async fn test_array_date_block_insert_basic() {
    let (mut client, db_name) =
        create_isolated_test_client("array_date_block_basic")
            .await
    ...
    // Create nested Date column (Date is stored as days since 1970-01-01)
    let mut nested = ColumnDate::new(Type::date());
    nested.append(0); // 1970-01-01
    nested.append(18993); // 2022-01-01
    nested.append(19358); // 2023-01-01

    // Create Array column with offsets: [2, 3] for arrays [[0, 18993],
    // [19358]]
...
#[tokio::test]
#[ignore]
async fn test_array_date_block_insert_boundary() {
    let (mut client, db_name) =
        create_isolated_test_client("array_date_block_boundary")
            .await
            .expect("Failed to create test client");

    client
        .query(format!(
            "CREATE TABLE {}.test_table (id UInt32, dates Array(Date)) ENGINE = Memory",
            db_name
        ))
        .await
        .expect("Failed to create table");
    ...
    let test_cases = vec![
        ...
        ("Multiple elements", vec![0, 18993, 19358]), /* 1970-01-01,
                                          * 2022-01-01,
                                          * 2023-01-01 */
        ("Min date", vec![0]), // 1970-01-01
        ("Recent dates", vec![19358, 19359, 19360]), /* 2023-01-01 to
                                * 2023-01-03 */
        ("Many dates", vec![18993, 18994, 18995, 18996, 18997]),
    ];
...
#[tokio::test]
#[ignore]
async fn test_array_date_block_insert_many_elements() {
    let (mut client, db_name) =
        create_isolated_test_client("array_date_block_many")
            .await
            .expect("Failed to create test client");
    ...
    // Create a large array with 365 consecutive days starting from 2020-01-01
    let mut nested = ColumnDate::new(Type::date());
    let start_date = 18262; // 2020-01-01
    for i in 0..365 {
...
proptest! {
    ...
    #[test]
    #[ignore]
    fn test_array_date_block_insert_random(
        arrays in prop::collection::vec(
            prop::collection::vec(0u16..25000u16, 0..20),  // Valid Date range
            1..10
        )
    ) {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let (mut client, db_name) = create_isolated_test_client("array_date_block_random")
                .await
                .expect("Failed to create test client");
